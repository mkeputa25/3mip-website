# 3MIP — Verified Facts

**This file is the single source of truth for every factual claim in the website.**

Rule: if a fact is in this file, use it verbatim. If a fact is not in this file, tag it `[VERIFY]` inline in the content and surface it in `OPEN-QUESTIONS.md` at the end of the relevant phase. Do not invent. Do not approximate. Do not "improve" a citation by adding a year you assumed.

Sources are cited inline. Where a source is contested or has been updated, the most recent verified source is used.

---

## Project identity

- **Full name**: Mobility and Migration Modeling Intercomparison Project
- **Acronym**: 3MIP
- **Acronym usage**: spell out on first reference per page; 3MIP thereafter
- **Tagline candidates** (Mario to choose, Bell to approve): "A community benchmark for climate-migration models" / "Comparing migration models on a shared question" / "Intercomparison for climate-migration modeling"
- **Launch date**: October 2025
- **Founding event**: Climate Migration Modeling Intercomparison Workshop, Princeton C-PREE, September 25–27, 2024
- **Current website**: 3mip.weebly.com (to be deprecated)
- **Official logo**: `final-3mip-logo.png` exists on the Weebly site. New-site treatment `[PENDING BELL APPROVAL]` — see OPEN-QUESTIONS.md. The typographic wordmark ships until decided; do not mix both.
- **First benchmark case**: coastal Bangladesh under climate stress

## Leadership

### Andrew Reid Bell (Co-Lead)

- **Affiliation**: Schleifer Family Professor of Sustainability, Department of Global Development, Cornell University
- **Appointed**: July 2024 (inaugural Schleifer Professor)
- **Source for title**: Cornell Chronicle, "Inaugural Schleifer Professor of Sustainability announced," February 2024
- **Prior**: Associate Professor, NYU (Department of Environmental Studies)
- **Role in 3MIP**: Co-lead, primary PI
- **Research focus**: Agent-based modeling of climate adaptation, migration, and food systems; developer of the MIDAS (Migration, Intensification, and Diversification as Adaptive Strategies) framework
- **Google Scholar citation count**: 3,539 as of May 2026 (note: profile shows outdated affiliation; verified current title via Cornell Einaudi Center directory)
- **Key publications relevant to 3MIP**:
  - Bell, A. R., Wrathall, D. J., Mueller, V., Chen, J., Oppenheimer, M., Hauer, M., Adams, H., Kulp, S., Clark, P. U., Fussell, E., Magliocca, N., Xiao, T., Gilmore, E. A., Abel, K., Call, M., & Slangen, A. B. A. (2021). "Migration towards Bangladesh coastlines projected to increase with sea-level rise through 2100." *Environmental Research Letters*, 16(2), 024045.
  - Bell, A. R., Calvo-Hernandez, C., & Oppenheimer, M. (2019). "Migration, Intensification, and Diversification as Adaptive Strategies." *Socio-Environmental Systems Modelling*, 1, 16102. DOI: 10.18174/sesmo.2019a16102
- **Email**: andrew.reid.bell@cornell.edu `[VERIFY current Cornell address — older publications cite andrew.reid.bell@nyu.edu]`
- **Cornell page**: https://einaudi.cornell.edu/discover/people/andrew-reid-bell
- **Personal site**: https://andrewreidbell.com (added Phase 4D)

### Kelsea Best (Co-Lead)

- **Affiliation**: Assistant Professor, Department of Civil, Environmental & Geodetic Engineering (joint appointment with Knowlton School of Architecture, City & Regional Planning), The Ohio State University
- **Appointed**: 2023
- **Source**: Knowlton School announcement, August 2023
- **PhD**: Earth and Environmental Sciences, Vanderbilt University, 2022
- **Dissertation topic**: agent-based modeling of coastal Bangladesh migration
- **Role in 3MIP**: Co-lead
- **Research focus**: Climate migration modeling, environmental migration in coastal Bangladesh, agent-based simulation
- **Google Scholar citation count**: 564 as of May 2026
- **Key publications relevant to 3MIP**:
  - Best, K., Ober, K., & McLeman, R. (2025). *Migration and Displacement in a Changing Climate*. Cambridge University Press.
- **OSU page**: https://si.osu.edu/kelsea-best
- **Personal site**: https://kelseabestresearch.com (added Phase 4D)

### Lars Tierolf (Co-Lead)

- **Affiliation**: PhD researcher, Institute for Environmental Studies (IVM), Vrije Universiteit Amsterdam
- **Affiliated project**: COASTMOVE (ERC Advanced Grant)
- **Role in 3MIP**: Co-lead
- **Research focus**: Coupled coastal adaptation and migration agent-based modeling
- **Key publications relevant to 3MIP**:
  - Tierolf, L., de Moel, H., van Vuuren, D. P., Botzen, W. J. W., Hoogvliet, M., Aerts, J. C. J. H. (2024). "A coupled agent-based model for France for simulating adaptation and migration decisions under future coastal flood risk." *Science of the Total Environment*, 917, 170239.
- **VU page**: https://research.vu.nl/en/persons/lars-tierolf (confirmed — Phase 4D crawl)

## Project management

- **Mario Keputa**: Project coordinator, Cornell University. Email: mk2674@cornell.edu.
- ~~Ishita Kashyap~~ — no longer part of the project (removed June 2026, per Mario). Do not list anywhere on the site.

## Advisory committee (14 members)

Each entry: name, current primary affiliation, one-line role in 3MIP (Mario to refine after Bell sign-off).

1. **Michael Oppenheimer** — Albert G. Milbank Professor of Geosciences and International Affairs, Princeton School of Public and International Affairs, Princeton University. IPCC Coordinating Lead Author, Special Report on the Ocean and Cryosphere in a Changing Climate (2019). 66,477 Google Scholar citations. Role: senior science advisor, methodological framing.

2. **Alex de Sherbinin** — Director, Center for International Earth Science Information Network (CIESIN), Columbia Climate School, Columbia University. Co-author, World Bank *Groundswell* reports. Role: data and policy interface advisor.

3. **Jeroen Aerts** — Professor, Institute for Environmental Studies (IVM), Vrije Universiteit Amsterdam. Principal Investigator, COASTMOVE (ERC Advanced Grant 884442, €2,499,608.75, January 2021 – June 2026). Source: CORDIS. Role: coastal modeling methods advisor.

4. **Roman Hoffmann** — Research Scholar, Population and Just Societies Program, International Institute for Applied Systems Analysis (IIASA), Laxenburg, Austria. Role: demographic and migration data advisor.

5. **Lisa Thalheimer** — Research Scholar, IIASA. Lead author, *Nature Communications* paper on involuntary immobility (2025). Role: immobility and trapped populations advisor.

6. **Nic Choquette-Levy** — Postdoctoral Research Associate, Penn State University. Co-organizer, Princeton C-PREE September 2024 founding workshop. Role: smallholder modeling advisor, founding workshop co-organizer.

7. **Fabien Cottier** — University of Geneva. Co-organizer, Princeton C-PREE September 2024 founding workshop. Role: empirical migration analysis advisor, founding workshop co-organizer.

8. **Michał Burzyński** — Senior researcher, Luxembourg Institute of Socio-Economic Research (LISER). Role: economic migration modeling advisor. (Spelling corrected to Polish diacritics in Phase 6; the earlier ASCII form was a flattening.)

9. **Michael Puma** — Director, Center for Climate Systems Research, Columbia University; NASA Goddard Institute for Space Studies. Role: food systems and climate impacts advisor.

10. **Thomas Bearpark** — Princeton University. Role: climate impacts and econometrics advisor. `[VERIFY exact current title]`

11. **Loring Thomas** — Princeton University. Role: `[VERIFY current title and 3MIP role]`

12. **Woi Sok Oh** — University of Waterloo. Role: socio-environmental modeling advisor. `[VERIFY exact title]`

13. **Seynabou Sall** — Initiative Prospective Agricole et Rurale (IPAR), Senegal. Role: West African case-study and field perspective advisor. `[VERIFY title]`

14. **Maxine Leis** — Uppsala University. Role: `[VERIFY title and 3MIP role]`

## Funding and institutional sponsors

### Confirmed and publicly disclosed

**Founding workshop sponsors (September 2024 Princeton C-PREE workshop):**

- Lamont-Doherty Earth Observatory (LDEO) Climate Center, Columbia University
- Carolee Bol and Scott Rosenberg '95 C-PREE Fund, Princeton University
- Department of Global Development, Cornell University
- United Nations Global Centre for Climate Mobility (GCCM)

Source: Princeton C-PREE workshop announcement, September 2024.

### Operating phase (2025–2027)

`[VERIFY — no publicly identifiable dedicated grant for 3MIP's 2025–2027 operating phase has been confirmed as of May 2026. The 2024 workshop summary indicated a planned NSF Research Coordination Network (RCN) application as next step. Status not public. Do not list specific operating-phase funders without explicit Bell sign-off.]`

### Adjacent grants of supporting team members (do not list as 3MIP funders)

- **COASTMOVE**: ERC Advanced Grant 884442, PI Jeroen Aerts, €2,499,608.75, January 2021 – June 2026. Source: CORDIS database, European Commission. This funds Lars Tierolf and Aerts' work; it does not fund 3MIP as such.
- **NSF Award #1934978** ("GCR: Disentangling Environmental Change and Social Factors as Drivers of Migration"), PI Alex de Sherbinin. This funds adjacent CIESIN work; not a 3MIP grant.

## Founding workshop

- **Title**: Climate Migration Modeling Intercomparison Workshop
- **Dates**: September 25–27, 2024
- **Location**: Princeton University, Center for Policy Research on Energy and the Environment (C-PREE)
- **Organizers**: Nic Choquette-Levy (Penn State), Fabien Cottier (Geneva)
- **Attendance**: 30+ researchers from three continents
- **Sponsors**: see "Founding workshop sponsors" above
- **Next-step output (per Princeton write-up)**: "develop a proposal for an NSF Research Coordination Network or similar opportunity to fund long-term (e.g. 3+ years) climate migration model intercomparison research"
- **Source**: Princeton C-PREE news, October 2024; Columbia CIESIN news, October 2024

## Timeline (as currently planned)

Corrected June 2026 (Phase 4D) against the official site. The Weebly
timeline's "Early November, 2026" is a typo for 2025. The earlier
February/March **2025** webinar dates in this file were wrong — they predate
the project launch; the recorded webinars are February and March **2026**
(year confirmed by Mario).

| Date | Event | Status |
|---|---|---|
| September 25–27, 2024 | Founding workshop, Princeton C-PREE | Completed |
| October 1, 2025 | 3MIP launch and recruitment opens | Completed |
| Early November 2025 | First participant webinar | Completed |
| December 2025 – May 2026 | Regular participant webinars | Completed/ongoing |
| **July 12–16, 2026** | **iEMSs 2026, University College Dublin — Session C7 + Workshop WSC7** | **Public inflection point** |
| August – December 2026 | Regular webinars continue | Planned |
| Late 2026 onward | Submissions to *Climatic Change* topical collection, received through 2027 | Aspirational — formal Springer call not yet posted |
| 2027+ | Additional benchmark cases | TBD |

### Webinar recordings (Drive links from the official site)

**Corrected in Phase 4e (V7.1):** webinars are titled by month per the
official site, and no speaker names are displayed on the website. Speaker
identities below are internal metadata only.

- **"February Webinar Presentation"** (February 2026) — speaker metadata: Hedda Bonatz (University of Kiel). Recording: https://drive.google.com/file/d/1UA-KpVOobspNNLXdW1pD4ctI4v0y9UJi/view
- **"March Webinar Presentation"** (March 2026). Recording: https://drive.google.com/file/d/1Of1ndcRHtXi54mnlKnaBDDHida04ak29/view
- **"May Webinar Presentation"** (May 2026 — confirmed by Mario, June 2026; supersedes the earlier provisional November dating). Speaker metadata: Andrew Bell (Cornell). Recording: https://drive.google.com/file/d/1-qAABRtk0wHcn95Gu0LjAypfRUkPIAqT/view

Hosting: recordings stay on Google Drive in v1 (see archive/v1/DECISIONS.md D20); linked as external buttons, never embedded iframes.

## iEMSs 2026 Dublin

- **Conference**: International Environmental Modelling and Software Society biennial conference 2026
- **Location**: University College Dublin, Dublin, Ireland
- **Dates**: July 12–16, 2026 (confirmed — Phase 4D, June 2026)
- **Preliminary program**: publishes June 15, 2026. Session day and room `[PENDING — program June 15]`
- **3MIP Session**: C7 — "Mobility and Migration Modeling Intercomparison Project (3MIP) – An open, first synthesis"
- **3MIP Workshop**: WSC7 — "Panel Discussion – 3MIP"
- **Source**: https://conference.iemss.org/streams/

## Case 1: Bangladesh — key facts

- **Country population**: 175.7 million. Source: United Nations Population Fund, *State of World Population 2025*. Launched in Bangladesh on 7 July 2025. Reported widely in Bangladeshi press (The Business Standard, Daily Star).
- **Working-age population share**: per UNFPA SWOP 2025, majority of population is in working-age group `[VERIFY exact percentage]`
- **Why this case**: empirical density (one of the most-studied coastal migration contexts globally), policy relevance, abundant open data, methodological precedent (Bell et al. 2021 *Environmental Research Letters*)
- **Geographic focus**: coastal districts of Bangladesh, including Khulna, Satkhira, Bagerhat, Barguna, Patuakhali, Pirojpur, Bhola, Noakhali, Chattogram (Chittagong), and adjacent districts in the Ganges-Brahmaputra-Meghna delta `[VERIFY exact district list against curated dataset documentation]`
- **Time horizon**: 2025–2050 baseline, with extensions to 2100 for some teams
- **Climate scenarios**: SSP2-4.5 baseline, with SSP5-8.5 and SSP1-2.6 as sensitivity scenarios `[VERIFY exact scenario specification from 3MIP protocol document — Mario to confirm with Bell]`

## Datasets curated for the Bangladesh case

**Replaced June 2026 (Phase 4D)** with the full 16-dataset list from a complete
crawl of the official 3mip.weebly.com data page — the earlier 7-item draft in
this file was partial. The Tellman citation is resolved (it is the flood
exposure paper, not forest loss; the Weebly site misspells "Telman" — use
Tellman). The BBS entry is resolved: full title is "Report on Socio-Economic
and Demographic Survey 2023."

**Data-extension policy (official wording, corrected in 4D):** "This data list
may be expanded, but any new dataset must be made known and available to all
teams. Contact project coordination to request the inclusion of an additional
dataset." The earlier "document your own additions in your submission"
framing was wrong — do not use it anywhere on the site.

### Population and socioeconomic data

1. **GHSL population density** — Population density, 1975–2030, 100m–1km. Schiavina, M., Freire, S., MacManus, K. (2019), European Commission JRC. CC BY 4.0. Data: https://human-settlement.emergency.copernicus.eu/download.php?ds=pop
2. **Building locations and footprint** — point data, 2024. Florio, P., et al. (2024), JRC Data Catalogue. `[VERIFY full citation]` Data: https://data.jrc.ec.europa.eu/dataset/f41a22f1-5741-4c41-86eb-6384654f6927
3. **Age by gender** — 30 arc-sec, 2010. CIESIN (2018), GPWv4 Basic Demographic Characteristics, Rev. 11, NASA SEDAC. CC BY 4.0. Data: https://www.earthdata.nasa.gov/data/catalog/sedac-ciesin-sedac-gpwv4-bdc-r11-4.11
4. **Income, wealth, rural/urban, age, gender, education** — admin units, 2015. Ton, M. J., et al. (2024), *Scientific Data*, DOI: 10.1038/s41597-024-03864-2. `[VERIFY full author list and title]` Data: https://doi.org/10.7910/DVN/KJC3RH
5. **Education** — admin units, 2010. Smits, J., & Permanyer, I. (2019), The Subnational Human Development Database, *Scientific Data*, 6, 190038. Data: https://globaldatalab.org/shdi/download/
6. **Income** — admin units, 2010. Smits & Permanyer (GDL SHDI), as above. Data: https://globaldatalab.org/shdi/download/
7. **Per capita income** — 5 & 30 arc-min, 1990–2015. Kummu, M., Taka, M., & Guillaume, J. H. A. (2018), *Scientific Data*, 5, 180004. Data: https://datadryad.org/dataset/doi:10.5061/dryad.dk1j0

### Environmental data

8. **Inundation** — 30 arc-sec; 2015, 2030, 2050, 2080. Ward, P. J., et al. (2020), WRI Aqueduct Floods. Data: https://wri-projects.s3.amazonaws.com/AqueductFloodTool/download/v2/index.html
9. **Flood extent and duration** — 250m, 2000–2018. Tellman, B., Sullivan, J. A., Kuhn, C., et al. (2021). "Satellite imaging reveals increased proportional population exposure to floods." *Nature*, 596, 80–86. DOI: 10.1038/s41586-021-03695-w. Data: http://global-flood-database.cloudtostreet.ai/#interactive-map
10. **Soil salinity** — 30 arc-sec, 1980–2018. Hassani, A., Azapagic, A., Shokri, N. (2020), *PNAS*, 117(52), 33017–33027. DOI: 10.1073/pnas.2013771117. CC BY 4.0. Data: https://doi.org/10.6084/m9.figshare.13295918.v1
11. **Soil salinity and effects on yield** — Upazila level, 2010. Soil Resource Development Institute, Government of the People's Republic of Bangladesh (SRDI report, November 2010). Data (exact URL from the archived Weebly data.html, resolved Phase 4e): https://srdi.portal.gov.bd/sites/default/files/files/srdi.portal.gov.bd/publications/bc598e7a_df21_49ee_882e_0302c974015f/Soil%20salinity%20report-Nov%202010.pdf
12. **Precipitation** — 0.01° gridded, 1901–2018. Dewan, A., et al. (2022), *Scientific Data*, DOI: 10.1038/s41597-022-01568-z. Data: https://figshare.com/articles/dataset/Long_term_high_resolution_gridded_rainfall_product_for_Bangladesh/16607912

### Mobility data

13. **Modeled internal migration flows** — district level, 2005–2010. Sorichetta, A., et al. (2016). "Mapping internal connectivity through human migration in malaria endemic countries." *Scientific Data*, 3, 160066. DOI: 10.1038/sdata.2016.66. CC BY 4.0. Data: https://doi.org/10.7910/DVN/RUWQQK/XGJNDL
14. **Number of people displaced** — event level, 2000–2019. Tellman, B., et al. (2021), as #9. Data: http://global-flood-database.cloudtostreet.ai/#interactive-map
15. **Number of people displaced** — XY data, 1985–2021. Brakenridge, G. R. (2021). Global Active Archive of Large Flood Events. Dartmouth Flood Observatory, University of Colorado. Data: https://floodobservatory.colorado.edu/
16. **Migration drivers** — district level, 2023. Bangladesh Bureau of Statistics (2023). *Report on Socio-Economic and Demographic Survey 2023*. Data: nsds.bbs.gov.bd

## Participation policy (from the official site, Phase 4D)

- **Two registration tracks**: (a) modeler; (b) expert — relevant expertise on Bangladesh and/or climate-related mobility.
- **Code and findings sharing**: "In participating, modelers will be asked to share their code and model findings." This is the official statement; the site must not contradict it. `[PENDING BELL APPROVAL if leadership wants softer public wording]`
- **Expert network**: regular meetings, scheduled to alternate time zones; experts provide domain consultation on Bangladesh and climate-related mobility.
- **Registration form also asks**: anyone else who might be interested in participating, or team members who should be included (names and emails).

## Domain experts (4 — from the official site, Phase 4D)

1. **Hedda Bonatz** — PhD researcher, Coastal Risk and Sea Level Rise group, University of Kiel. Spatial patterns of population development and flood-induced mobility. https://www.crslr.de/team/hedda-bonatz
2. **Khandaker Jafor Ahmed** — University of Michigan. Climate-migration research on environmental stressors in LMICs; BEMS dataset. https://seas.umich.edu/research/faculty/khandaker-jafor-ahmed
3. **Muhammad Abdur Rahaman** — Director, Center for People & Environ (CPE), Bangladesh. Disaster risk reduction and Nature-based Solutions. https://cpe-bd.org/2024/08/11/muhammad-abdur-rahaman/
4. **Ramesh Poluru** — The INCLEN Trust International. Demography and epidemiology; large-scale socioeconomic and behavioral studies. https://scholar.google.com/citations?user=xT4xVZAAAAAJ

No photos in v1 `[PENDING consent confirmation — photos exist in the Weebly archive if approved]`.

## Key publications by the team (recent, relevant to 3MIP)

- Bell, A. R. et al. (2021). "Migration towards Bangladesh coastlines projected to increase with sea-level rise through 2100." *Environmental Research Letters*, 16(2), 024045. DOI: 10.1088/1748-9326/abdc5b
- Bell, A. R., Calvo-Hernandez, C., & Oppenheimer, M. (2019). "Migration, Intensification, and Diversification as Adaptive Strategies." *Socio-Environmental Systems Modelling*, 1, 16102. DOI: 10.18174/sesmo.2019a16102
- Tierolf, L. et al. (2024). "A coupled agent-based model for France for simulating adaptation and migration decisions under future coastal flood risk." *Science of the Total Environment*, 917, 170239. DOI: 10.1016/j.scitotenv.2024.170239
- Best, K., Ober, K., & McLeman, R. (2025). *Migration and Displacement in a Changing Climate*. Cambridge University Press.
- Hauer, M. E., Saunders, D. C., Shtob, D. (2021). "Migration towards Bangladesh coastlines projected to increase with sea-level rise through 2100." Source for Hauer et al. version of the analysis: https://mathewhauer.com/publication/2021-01-01_migration_towards_ba/ — same paper as Bell et al. 2021 above, with Hauer as co-author.
- Reimann, L., Vafeidis, A. T., Honsel, L. E. (2023). "Population development as a driver of coastal risk: Current trends and future pathways." *Nature Communications*, 14(1). `[VERIFY — useful comparison reference; not a 3MIP output]`

## Adjacent and reference projects

- **AgMIP** (Agricultural Model Intercomparison and Improvement Project): agmip.org. Started 2010. Multi-institutional. Direct organizational analogue for 3MIP.
- **ISIMIP** (Inter-Sectoral Impact Model Intercomparison Project): isimip.org. Coordinated by PIK Potsdam. Direct organizational analogue.
- **CMIP6** (Coupled Model Intercomparison Project Phase 6): the gold standard intercomparison framework in climate science. Reference for "what good intercomparison looks like."
- **COASTMOVE**: coastmove.wordpress.com. Tierolf's home project. Complementary, not competing.
- **HABITABLE** (EU Horizon 2020): adjacent climate migration consortium, more policy and empirics focused.
- **MISTY**, **PRISE**: smaller adjacent consortia.
- **World Bank Groundswell reports** (Rigaud, de Sherbinin, Jones, Bergmann, et al., 2018; 2021): top-down country-level migration projections. 3MIP outputs would feed bottom-up alternatives.
- **UN Global Centre for Climate Mobility (GCCM)**: policy interface partner for the founding workshop.

## Anti-facts (do not state these — they have appeared on AI-generated drafts of this material and are wrong)

- 3MIP is **not** an NSF-funded project as of May 2026. NSF RCN application is planned, status not public.
- 3MIP is **not** affiliated with the IPCC.
- 3MIP is **not** a UN program. The UN GCCM co-sponsored the founding workshop only.
- The founding workshop was **not** held at Cornell. It was held at Princeton C-PREE.
- Bell is **no longer at NYU**. He moved to Cornell in July 2024.
- The first benchmark case is **coastal Bangladesh specifically**, not "South Asia" or "the Ganges Delta" generally.
- 3MIP does **not** produce migration forecasts. It coordinates intercomparison among independent modeling teams. The teams produce outputs; 3MIP synthesizes.
- The *Climatic Change* topical collection is **planned, not open**. Springer has not posted a formal call as of May 2026.

## How to cite 3MIP

`[Mario to confirm with Bell — proposed format below]`

Suggested citation for the curated input data package:
> Bell, A. R., Best, K., Tierolf, L., & the 3MIP Consortium (2025). *3MIP Curated Input Dataset for Coastal Bangladesh, Version 1.0*. Mobility and Migration Modeling Intercomparison Project. [DOI when minted].

Suggested citation for the project as a whole:
> Mobility and Migration Modeling Intercomparison Project (3MIP). https://3mip.org `[PENDING BELL APPROVAL on final domain]`

## Verification log

Every fact in this file has been cross-referenced against at least one publicly available source as of the analysis date. Where verification is incomplete, the `[VERIFY]` tag is present inline. Where a fact depends on a private decision (e.g., final domain), the `[PENDING BELL APPROVAL]` tag is present.

When Claude Code adds a content claim during the build, it must either:
1. Cite a fact in this file, or
2. Add `[VERIFY]` inline AND surface the unverified claim in `OPEN-QUESTIONS.md` at the end of the phase.

Never proceed past a `[VERIFY]` tag silently. The whole point of this file is to keep the website's factual ground sound.

**June 2026 approvals (relayed by Mario):** Bell sign-off on the shipping
defaults — hero headline, tagline, terracotta system, Fraunces wordmark
(B11 resolved: wordmark over the old logo), registration-gated protocol,
official code-sharing wording, press-via-Cornell-Relations, privacy and
accessibility notice wording, curated-dataset citation format, signature
figure (archetypes, patterns, motion, lineage attributions), and leadership
+ coordinator photos (B3: monochrome treatment; Bell/Best/Tierolf live,
Keputa pending file drop). Ishita Kashyap removed from the project. The
Bell webinar recording is May 2026. Still genuinely open: final domain
(B1), operating-phase funder announcements (B4), GitHub repo URL (B18),
and the factual [VERIFY] items listed in OPEN-QUESTIONS.md § 2.

**Phase 4D update (June 2026):** full crawl of 3mip.weebly.com (all 7 pages)
by Mario. Corrections: timeline (Weebly "Early November, 2026" typo → 2025;
webinar recordings are February/March 2026, not 2025), iEMSs 2026 dates
confirmed (July 12–16, University College Dublin; preliminary program June
15), Tellman citation resolved (flood exposure paper), BBS survey title
resolved, dataset list replaced with the complete 16-item official list.
Additions: dual-track participation policy, code-and-findings-sharing policy,
data-extension policy, domain experts (4), leads' personal sites, official
logo existence. The live site is the official record; where this file
conflicted with it, this file was corrected.

**Phase 4e update (June 2026, V7):** webinars retitled by month ("February
Webinar Presentation" etc.) with no speaker names displayed — speaker
identities kept as internal metadata; the Simona-surname `[VERIFY]` is moot
under month naming. SRDI exact PDF URL resolved from the archived Weebly
data.html. Mallick, Best, Carrico, Ghosh, Priodarshini, Sultana & Samanta
(2023) title confirmed: "How do migration decisions and drivers differ
against extreme environmental events?" *Environmental Hazards*,
DOI 10.1080/17477891.2023.2195152.
